...
<div class="card shadow-sm border-0">
    ...
    <form id="form-tambah" method="post" action="../proses/proses_edit_transaksi.php" class="row g-3">
        ...
        <div class="col-md-6"><label for="jenis" class="form-label">Jenis Transaksi</label><select id="jenis" name="jenis" class="form-select" data-category-target="kategori" required><option value="masuk"<?php echo $transaksi['jenis'] === 'masuk' ? ' selected' : ''; ?>>Kas Masuk</option><option value="keluar"<?php echo $transaksi['jenis'] === 'keluar' ? ' selected' : ''; ?>>Kas Keluar</option></select></div>
        <div class="col-md-6"><label for="kategori" class="form-label">Kategori</label><select id="kategori" name="kategori" class="form-select" data-selected="<?php echo htmlspecialchars($transaksi['kategori']); ?>" required></select></div>
