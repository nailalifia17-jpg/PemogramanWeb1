...
{
    $sql = 'SELECT id, tanggal, bulan, tahun, keterangan, jenis, kategori, jumlah, metode FROM transaksi';
    $conditions = [];
    $params = [];
...
function ambilTransaksiHalaman(PDO $db, string $q, ?int $bulan, ?int $tahun, int $limit, int $offset): array
{
    ...
    $statement = $db->prepare('SELECT id, tanggal, bulan, tahun, keterangan, jenis, kategori, jumlah, metode FROM transaksi WHERE ' . implode(' AND ', $conditions) . ' ORDER BY tanggal DESC, id DESC LIMIT :limit OFFSET :offset');
...
{
    $statement = $db->prepare('SELECT id, tanggal, bulan, tahun, keterangan, jenis, kategori, jumlah, metode FROM transaksi WHERE id = :id');
    $statement->execute([':id' => $id]);
    return $statement->fetch() ?: null;
}
